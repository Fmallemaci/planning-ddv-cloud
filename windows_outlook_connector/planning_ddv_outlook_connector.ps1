...
$tempDir = $null
try {
    Write-ConnectorLog "Inicio" "Version $ConnectorVersion - URL recibida: $ProtocolUrl"
    ...
    if ([string]::IsNullOrWhiteSpace($packageUrl)) {
        ...
        $BaseUrlFile = Join-Path $InstallDir "base_url.txt"
        if ([string]::IsNullOrWhiteSpace($BaseUrl) -and (Test-Path -LiteralPath $BaseUrlFile)) {
